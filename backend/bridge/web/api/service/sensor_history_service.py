import numpy as np
from web.models import Sensor, MinuteHistoryData, QuartileData, DailyHistoryData, WeatherDailyHistoryData, Bridge
...
class SensorHistoryService:
    ...
    def get(self, request: Request, bid: int, sid: int):
        """
        ...
        try:
            ...
            data = self.__get_data_from_custom_range(bid, sid, time_dict, filtered_features)
    ...
    def __get_data_from_custom_range(self, bid: int, sid: int , time_dict: dict, filtered_features: list):
        """
        ...
        if (time_dict["time_range"] <= timedelta(days = 1)):
            ...
            daily_filtered = DailyHistoryData.objects.filter(
                bid= bid,
                sensor= sid,
                time__range=(time_dict["start_date"], time_dict["end_date"])
            ).annotate(
    ...
    def __get_filtered_fields(self, bid: int, sid: int):
        ...
        all_features = [field.name for field in MinuteHistoryData._meta.get_fields()]
